...
import { executerGeneration } from "@/lib/generation-contenu";
...
function PageGenererInner() {
  ...
  async function generer(params: ParamsGeneration | Record<string, any>) {
    ...
    // ── Tous les autres types : le répartiteur partagé ─────────────────
    // Le même code sert au panneau « Depuis ma programmation », qui régénère
    // un exercice à l'unité avec ces formulaires.
    try {
      const c = await executerGeneration(params);
      setContenu(c as ContenuPreview);
      setEtape("apercu");
    } catch (e) {
