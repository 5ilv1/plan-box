...
export default function NouvelleSemainePage() {
  ...
  const [modalPeriodicite, setModalPeriodicite] = useState<"jour" | "semaine">("jour");
  ...
  async function supprimerBloc(id: string) {
    ...
    // Si c'est un bloc écriture semaine (J1), supprimer aussi les repères visuels J2/J3/J4
    if (bloc.type === "ecriture" && (bloc.contenu as any)?._periodicite === "semaine") {
      setBlocs((prev) => prev.filter((b) => {
        if (b.id === id) return false;
        // Supprimer les blocs visuels écriture associés
        if (b.type === "ecriture" && (b.contenu as any)?._visual_only) return false;
        return true;
      }));
    } else {
  ...
  function ouvrirModale(bloc: BlocSemaine) {
    ...
    setModalBanqueId("");
    setModalPeriodicite(bloc.contenu?._periodicite === "semaine" ? "semaine" : "jour");
  ...
  async function sauverModale() {
    ...
    if (modalBloc.type === "ecriture" && modalPeriodicite === "semaine" && modalBanqueId) {
      ...
      setBlocs((prev) => {
        ...
        for (let i = 0; i < JOURS_ATELIER.length; i++) {
          const ja = JOURS_ATELIER[i];
          const isFirst = i === 0;
          newBlocs.push({
            id: uid(),
            type: "ecriture" as TypeBloc,
            titre: `${titreBase} — ${ja.label}`,
            jour: ja.jour,
            assignation,
            chapitreId: modalChapitreId || undefined,
            contenu: isFirst
              ? { banque_id: modalBanqueId, _periodicite: "semaine" }
              : { _visual_only: true, _atelier_jour: i + 1 },
          });
        }
        return [...without, ...newBlocs];
      });
    ...
    setBlocs((prev) => prev.map((b) => {
      ...
      // Propager la periodicite pour les blocs semaine (écriture, ressource)
      if (modalPeriodicite === "semaine") {
        contenu._periodicite = "semaine";
      }
  ...
  async function genererEtSauvegarder(params: any) {
    ...
    try {
      ...
      setModalTitre(titre);
      setModalPeriodicite(contenuData._periodicite === "semaine" ? "semaine" : "jour");
      setShowExoGenerateur(false);

      // ── Écriture semaine : créer directement les 4 blocs sur la grille ──
      if (type === "ecriture" && contenuData._periodicite === "semaine" && modalBloc) {
        const assignation: AssignationSelecteur = modalAssignation === "classe"
          ? { groupeIds: groupesPB.map((g) => g.id), eleveUids: [], groupeNoms: groupesPB.map((g) => g.nom) }
          : { groupeIds: modalAssignation as string[], eleveUids: [], groupeNoms: groupesPB.filter((g) => (modalAssignation as string[]).includes(g.id)).map((g) => g.nom) };
        ...
        setBlocs((prev) => {
          ...
          for (let i = 0; i < JOURS_ATELIER.length; i++) {
            ...
            newBlocs.push({
              id: uid(),
              type: "ecriture" as TypeBloc,
              titre: `${titre} — ${ja.label}`,
              jour: ja.jour,
              assignation,
              chapitreId: modalChapitreId || undefined,
              contenu: i === 0
                ? { banque_id: jsonB.id, _periodicite: "semaine" }
                : { _visual_only: true, _atelier_jour: i + 1 },
            });
          }
          return [...without, ...newBlocs];
    ...
    try {
      const res = await fetch("/api/planifier-semaine", {
        ...
        body: JSON.stringify({
          lundi: lundiProchain,
          blocs: blocs
            .filter((b) => !(b as any)._existant && !(b.contenu as any)?._visual_only) // Ne pas renvoyer les blocs déjà en base ni les repères visuels
            .map((b) => ({
              type: b.type,
              titre: b.titre,
              jour: b.jour,
                      ...
                      >
                        ...
                        <div style={{ flex: 1, padding: 8, display: "flex", flexDirection: "column", gap: 6 }}>
                          {blocsJour.map((bloc) => {
                            ...
                            const isVisualOnly = (bloc.contenu as any)?._visual_only === true;
                            const atelierJour = (bloc.contenu as any)?._atelier_jour as number | undefined;
                            const nbEleves = (bloc as any)._nbEleves ?? 0;
                            const nbFaits = (bloc as any)._nbFaits ?? 0;

                            // Bloc repère visuel écriture semaine (J2/J3/J4)
                            if (isVisualOnly && atelierJour) {
                              return (
                                <div
                                  key={bloc.id}
                                  style={{
                                ...
                                >
                                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                                    <span className="ms" style={{ fontSize: 14, color: "#7C3AED" }}>edit_note</span>
                                  ...
                                  <div style={{ fontSize: 9, color: "#7C3AED", fontStyle: "italic", marginTop: 2 }}>
                                    Suite de l&apos;atelier semaine
                                  </div>
                                </div>
                              );
                            }
